...
public class ExpressionConvertor {
	public static String infixToPostfix(List<String> split) {
		...
		//Action on the ArrayList Of Expression
		replaceVariables(splitStr); 
	...
	public static void replaceVariables(List<String> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			//Check every index if is it a Var and then replace him
			if((v=Utilities.getVar(list.get(i))) != null) {
				//Check if the number negative if yes then its push - and the 
				//var to the LinkList
				if(v.calculate() >= 0)
					{
	...
	public static void main(String[] args) {
		String str = infixToPostfix(Arrays.asList("7","*","-","5"));
